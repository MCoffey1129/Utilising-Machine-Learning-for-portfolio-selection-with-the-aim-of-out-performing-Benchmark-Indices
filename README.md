...
*	The test data for which the training model is tested on consists of one trading window at Jul ’20 (c. 12.5% of the data)  
*	The training model is subsequently tested on the deploy dataset  to ensure that the models perform well across multiple time points (c. 12.5% of the data). Please note this trading window is only 4 months as we do not have data as of the last trading day of July. 
...
*	The “Diversified” portfolio simply takes the stock with the highest probability from each industry and combines each of these into an overall well diversified portfolio (this was completed using the Combined model). Please note there is a slight code update which is required (see bottom of this file) 
...
*	The Stacked model performs very well on the “Test” dataset but not as well in the “Deployment” dataset.   
...
Other interesting findings (deployment dataset):  
